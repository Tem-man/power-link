<template>
  <div class="connector-example">
    ...
    <!-- <div class="controls">
      <button @click="programmaticConnect" class="btn btn-primary"> 编程方式建立连接 </button>
      <button @click="programmaticDisconnect" class="btn btn-danger"> 编程方式断开连接 </button>
...
<script setup>
  ...
  const programmaticConnect = () => {
    if (connector && connector.nodes.length >= 2) {
      ...
      addLog("通过编程方式建立了多条连接（自动选择触点）", "success");
    }
  };
