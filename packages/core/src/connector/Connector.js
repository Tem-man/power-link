...
class Connector {
    ...
    bindZoomEvents() {
        this.handleWheel = (e) => {
            ...
            const newScale = Math.max(
                this.config.minZoom,
                Math.min(this.config.maxZoom, this.viewState.scale + delta)
            );
    ...
    setViewState(state = {}) {
        if (typeof state.scale === 'number') {
            ...
            this.viewState.scale = Math.max(
                this.config.minZoom,
                Math.min(this.config.maxZoom, state.scale)
            );
        }
    ...
    handleMouseUp = (e) => {
        ...
        // 优先使用吸附的目标节点
        let targetNode = this.isSnapped ?
            this.snapTarget :
            this.getNodeAtPosition(e.clientX, e.clientY);
    ...
    getDotPosition(element, dotPosition) {
        ...
        // 计算相对于容器的位置（屏幕坐标）
        const screenX =
            dotPosition === 'right' ? rect.right - containerRect.left : rect.left - containerRect.left;
        const screenY = rect.top + rect.height / 2 - containerRect.top;
